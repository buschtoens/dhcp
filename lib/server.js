...
function DHCP(address, onListening) {
  ...
  for(var fn in this._on)
    this.on[fn] = this._on[fn].bind(this);
  for(var fn in this.handle)
    this.handle[fn] = this.handle[fn].bind(this);


  this.socket = dgram.createSocket("udp4");
  this.socket.bind(67, this.address);
...
, messageAnswer: function(msg, answer) {
  buf = [
      answer.op || 0x01
    , answer.htype || 0x01
    , answer.hlen || 0x06
    , answer.hops || 0x00
    , answer.xid || msg.xid
    , answer.secs || msg.secs
    , answer.flags || 0x0000
    , answer.ciaddr || 0x00000000
    , answer.yiaddr || 0x00000000
    , answer.siaddr || 0x00000000
    , answer.giaddr || 0x00000000
    , answer.chaddr || msg.chaddr
  ];
  for(var i = 0; i < 192; i++) buf.push(0x00);
  buf.push(answer.magicCookie || msg.magicCookie);
  
  answer.options.forEach(function(value, key) {
    buf.push(key);
    buf.push(value.length);
    (new Buffer(value)).forEach(function(v) {
      buf.push(v);
    });
  });

  this.socket.send(buf, 0, buf.length, 68, msg.ciaddr && msg.ciaddr != "0.0.0.0" ? msg.ciaddr : "255.255.255.255");
}
, message: function(msg, rinfo) {
    msg.rinfo = rinfo;
    msg.answer = this.on.messageAnswer.bind(this, msg);
